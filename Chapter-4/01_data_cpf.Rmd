...
# Load raw comparative panel file (CPF)
data_cpf <- import(".../CPFv1.5.dta")
...
data_cpf <- data_cpf %>%
  group_by(orgpid) %>%
  fill(c("orgpid", "hid", "country", "female", "yborn", "edu4", "migr"), .direction = "downup") %>%
...
# ill_health
data_cpf <- data_cpf %>%
  mutate(poor_health = case_when(
    srh5 %in% c(4, 5) ~ 1,
    srh5 %in% c(1, 2, 3) ~ 0),
    poor_health = as.character(poor_health)) %>%
  select(-srh5)
...
data_cpf <- data_cpf %>%
  group_by(orgpid) %>%
  mutate(ret_corrected = ifelse(country == "Germany" & employment_status != "Employed" & lead(pension, 1) == 1, 1, retired),
         retired = ifelse(country == "Germany" & !is.na(ret_corrected), ret_corrected, retired)) %>%
  select(-ret_corrected)
...
unemp <- read.xlsx(".../unemployment_rate.xlsx")

unemp <- unemp %>% mutate(year = as.numeric(year))

data_cpf <- data_cpf %>% left_join(unemp, by = c("country", "year"))

# add annual inflation index
infl <- read.xlsx(".../inflation_rates.xlsx")

infl <- infl %>% mutate(year = as.numeric(year))

data_cpf <- data_cpf %>% left_join(infl, by = c("country", "year"))
...
data_cpf <- data_cpf %>% select(country, orgpid, hid, age, year, female, year_born, cohort_born, retired, migrant, education_level, marital_status, partner_in_hh, children_any, household_size, poor_health, employment_status, hh_income_eq, occupation_group, public_sector, self_employed, unemployment_rate, inflation_rate)

```

```{r}
save.image("data/data_preprocessed/data_cpf.RData")
