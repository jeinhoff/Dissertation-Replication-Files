...
```{r}
weight <- import(".../ppathl.dta")

weight <- weight %>% 
  mutate(orgpid = as.character(pid), weight = phrf) %>%
  select(orgpid, syear, weight) 

data_de <- data_de %>% left_join(weight, by = c("orgpid", "year" = "syear"))
...
# homeowner_type
# region
soep_hh_1 <- import("data/data_raw/data_raw_de/hl.dta")
soep_hh_1 <- soep_hh_1 %>% select(hid, syear, hlf0087_h)

soep_hh_2 <- import("data/data_raw/data_raw_de/hgen.dta")
...
```{r}
# drop duplicate rows
data_de <- data_de %>% group_by(orgpid, age) %>% slice(1)
...
vars <- data_de %>% 
  group_by(orgpid) %>% 
  filter(age == min(age)) %>% 
  select(orgpid, weight, hid) %>% 
  slice(1) %>%
  ungroup()

data_de <- data_de %>% select(-c("weight", "hid")) %>% left_join(vars, by = "orgpid")

rm(vars)

data_de <- data_de %>% select(country, orgpid, hid, age, retired, homeowner, homeowner_type, social_housing, region, female, year_born, migrant, education_level, marital_status, children_any, household_size, poor_health, occupation_group, hh_income_eq, public_sector, self_employed, unemployment_rate, inflation_rate, weight)

```

```{r}
save.image(".../data_de.RData")
