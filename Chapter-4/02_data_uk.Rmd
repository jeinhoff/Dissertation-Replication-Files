...
# Load comparative panel file (CPF)
load(".../data_cpf.RData")
...
w1 <- w1 %>% select(pidp, ba_pno, ba_ppno, ba_hidp, ba_istrtdaty, ba_indin91_xw)
colnames(w1) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 2 (BHPS)
w2 <- import(paste0(path_data, "bhps/bb_indresp.dta"))
w2 <- w2 %>% select(pidp, bb_pno, bb_ppno, bb_hidp, bb_istrtdaty, bb_indin91_xw)
colnames(w2) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 3 (BHPS)
w3 <- import(paste0(path_data, "bhps/bc_indresp.dta"))
w3 <- w3 %>% select(pidp, bc_pno, bc_ppno, bc_hidp, bc_istrtdaty, bc_indin91_xw)
colnames(w3) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 4 (BHPS)
w4 <- import(paste0(path_data, "bhps/bd_indresp.dta"))
w4 <- w4 %>% select(pidp, bd_pno, bd_ppno, bd_hidp, bd_istrtdaty, bd_indin91_xw)
colnames(w4) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 5 (BHPS)
w5 <- import(paste0(path_data, "bhps/be_indresp.dta"))
w5 <- w5 %>% select(pidp, be_pno, be_ppno, be_hidp, be_istrtdaty, be_indin91_xw)
colnames(w5) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 6 (BHPS)
w6 <- import(paste0(path_data, "bhps/bf_indresp.dta"))
w6 <- w6 %>% select(pidp, bf_pno, bf_ppno, bf_hidp, bf_istrtdaty, bf_indin91_xw)
colnames(w6) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 7 (BHPS)
w7 <- import(paste0(path_data, "bhps/bg_indresp.dta"))
w7 <- w7 %>% select(pidp, bg_pno, bg_ppno, bg_hidp, bg_istrtdaty, bg_indin91_xw)
colnames(w7) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 8 (BHPS)
w8 <- import(paste0(path_data, "bhps/bh_indresp.dta"))
w8 <- w8 %>% select(pidp, bh_pno, bh_ppno, bh_hidp, bh_istrtdaty, bh_indin91_xw)
colnames(w8) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 9 (BHPS)
w9 <- import(paste0(path_data, "bhps/bi_indresp.dta"))
w9 <- w9 %>% select(pidp, bi_pno, bi_ppno, bi_hidp, bi_istrtdaty, bi_indin99_xw)
colnames(w9) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 10 (BHPS)
w10 <- import(paste0(path_data, "bhps/bj_indresp.dta"))
w10 <- w10 %>% select(pidp, bj_pno, bj_ppno, bj_hidp, bj_istrtdaty, bj_indin99_xw)
colnames(w10) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 11 (BHPS)
w11 <- import(paste0(path_data, "bhps/bk_indresp.dta"))
w11 <- w11 %>% select(pidp, bk_pno, bk_ppno, bk_hidp, bk_istrtdaty, bk_indin01_xw)
colnames(w11) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 12 (BHPS)
w12 <- import(paste0(path_data, "bhps/bl_indresp.dta"))
w12 <- w12 %>% select(pidp, bl_pno, bl_ppno, bl_hidp, bl_istrtdaty, bl_indin01_xw)
colnames(w12) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 13 (BHPS)
w13 <- import(paste0(path_data, "bhps/bm_indresp.dta"))
w13 <- w13 %>% select(pidp, bm_pno, bm_ppno, bm_hidp, bm_istrtdaty, bm_indin01_xw)
colnames(w13) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 14 (BHPS)
w14 <- import(paste0(path_data, "bhps/bn_indresp.dta"))
w14 <- w14 %>% select(pidp, bn_pno, bn_ppno, bn_hidp, bn_istrtdaty, bn_indin01_xw)
colnames(w14) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 15 (BHPS)
w15 <- import(paste0(path_data, "bhps/bo_indresp.dta"))
w15 <- w15 %>% select(pidp, bo_pno, bo_ppno, bo_hidp, bo_istrtdaty, bo_indin01_xw)
colnames(w15) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 16 (BHPS)
w16 <- import(paste0(path_data, "bhps/bp_indresp.dta"))
w16 <- w16 %>% select(pidp, bp_pno, bp_ppno, bp_hidp, bp_istrtdaty, bp_indin01_xw)
colnames(w16) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 17 (BHPS)
w17 <- import(paste0(path_data, "bhps/bq_indresp.dta"))
w17 <- w17 %>% select(pidp, bq_pno, bq_ppno, bq_hidp, bq_istrtdaty, bq_indin01_xw)
colnames(w17) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 18 (BHPS)
w18 <- import(paste0(path_data, "bhps/br_indresp.dta"))
w18 <- w18 %>% select(pidp, br_pno, br_ppno, br_hidp, br_istrtdaty, br_indin01_xw)
colnames(w18) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 19 (UKHLS)
w19 <- import(paste0(path_data, "ukhls/a_indresp.dta"))
w19 <- w19 %>% select(pidp, a_pno, a_ppno, a_hidp, a_intdaty_dv, a_indinus_xw)
colnames(w19) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 20 (UKHLS)
w20 <- import(paste0(path_data, "ukhls/b_indresp.dta"))
w20 <- w20 %>% select(pidp, b_pno, b_ppno, b_hidp, b_intdaty_dv, b_indinub_xw)
colnames(w20) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 21 (UKHLS)
w21 <- import(paste0(path_data, "ukhls/c_indresp.dta"))
w21 <- w21 %>% select(pidp, c_pno, c_ppno, c_hidp, c_intdaty_dv, c_indinub_xw)
colnames(w21) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 22 (UKHLS)
w22 <- import(paste0(path_data, "ukhls/d_indresp.dta"))
w22 <- w22 %>% select(pidp, d_pno, d_ppno, d_hidp, d_intdaty_dv, d_indinub_xw)
colnames(w22) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 23 (UKHLS)
w23 <- import(paste0(path_data, "ukhls/e_indresp.dta"))
w23 <- w23 %>% select(pidp, e_pno, e_ppno, e_hidp, e_intdaty_dv, e_indinub_xw)
colnames(w23) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 24 (UKHLS)
w24 <- import(paste0(path_data, "ukhls/f_indresp.dta"))
w24 <- w24 %>% select(pidp, f_pno, f_ppno, f_hidp, f_intdaty_dv, f_indinui_xw)
colnames(w24) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 25 (UKHLS)
w25 <- import(paste0(path_data, "ukhls/g_indresp.dta"))
w25 <- w25 %>% select(pidp, g_pno, g_ppno, g_hidp, g_intdaty_dv, g_indinui_xw)
colnames(w25) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 26 (UKHLS)
w26 <- import(paste0(path_data, "ukhls/h_indresp.dta"))
w26 <- w26 %>% select(pidp, h_pno, h_ppno, h_hidp, h_intdaty_dv, h_indinui_xw)
colnames(w26) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 27 (UKHLS)
w27 <- import(paste0(path_data, "ukhls/i_indresp.dta"))
w27 <- w27 %>% select(pidp, i_pno, i_ppno, i_hidp, i_intdaty_dv, i_indinui_xw)
colnames(w27) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 28 (UKHLS)
w28 <- import(paste0(path_data, "ukhls/j_indresp.dta"))
w28 <- w28 %>% select(pidp, j_pno, j_ppno, j_hidp, j_intdaty_dv, j_indinui_xw)
colnames(w28) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 29 (UKHLS)
w29 <- import(paste0(path_data, "ukhls/k_indresp.dta"))
w29 <- w29 %>% select(pidp, k_pno, k_ppno, k_hidp, k_intdaty_dv, k_indinui_xw)
colnames(w29) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")

# Wave 30 (UKHLS)
w30 <- import(paste0(path_data, "ukhls/l_indresp.dta"), encoding = "UTF-8")
w30 <- w30 %>% select(pidp, l_pno, l_ppno, l_hidp, l_intdaty_dv, l_indinui_xw)
colnames(w30) <- c("orgpid", "person_n", "partner_n", "hid", "year", "weight")
...
data_uk <- data_uk %>% select(-hid) %>% unique() %>% left_join(waves %>% select(orgpid, hid, year, partner_n, weight) %>% unique(), by = c("orgpid", "year"))

# Merge with main
partner_employment <- import("data/data_raw/data_raw_cpf/CPFv1.5.dta")
...
partner_employment <- partner_employment %>% 
  unique() %>% 
  left_join(waves, by = c("orgpid", "year"))

partner_employment <- partner_employment %>% 
  ungroup() %>% 
  select(hid, year, person_n, emplst6) %>% 
  rename("employment_status_partner" = "emplst6") %>% 
  na.omit()
...
```{r}
# drop duplicate rows
data_uk <- data_uk %>% group_by(orgpid, age) %>% slice(1)
...
weights <- data_uk %>% 
  ...
  filter(age == min(age)) %>% 
  select(orgpid, weight) %>% ungroup()

data_uk <- data_uk %>% select(-weight) %>% left_join(weights, by = "orgpid")

rm(weights)

households <- import(".../xhhrel.dta")

households <- households %>% select(pidp, osm_hh) %>% rename("orgpid" = "pidp", "hid" = "osm_hh")

data_uk <- data_uk %>% mutate(orgpid = as.numeric(as.character(orgpid))) %>% select(-hid) %>% left_join(households, by = "orgpid")

rm(households, waves_combined)

data_uk <- data_uk %>% select(country, orgpid, hid, year, age, retired, homeowner, homeowner_type, social_housing, region, female, year_born, migrant, education_level, marital_status, children_any, household_size, poor_health, occupation_group, hh_income_eq, public_sector, self_employed, unemployment_rate, inflation_rate, weight)

```

```{r}
save.image(".../data_uk.RData")
