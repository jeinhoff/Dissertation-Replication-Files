...
load("/Users/jlein/Documents/analysis_extension/outputs/data_raw_55_64.RData")
...
survival <- survival %>% filter(cohort %in% c(1900:1990) & age %in% c(55:64))

t <- survival %>% filter(country == "LV")
...
```{r}
data <- data %>% filter(age %in% c(55:64))
...
table <- table %>%
  ...
  filter(n == 10 | cohort >= max_year - 64) %>%
  unique()
...
```{r}
data <- data %>% 
  select(country, country_code, country_group, gender, age, year, cohort, cohort_two, cohort_two_nb, status, working_hours, working_hours_usual, survival, weight) %>%
...
```{r}
data <- data %>% 
  filter(!is.na(country) & !is.na(country_code) & !is.na(country_group) & !is.na(gender) & !is.na(age) & !is.na(year) & !is.na(cohort) & !is.na(cohort_two) & !is.na(cohort_two_nb) & !is.na(status) & !is.na(survival) & !is.na(weight)) %>%
  filter(!(status == "Employed" & is.na(working_hours_usual)))

save.image("outputs/data_final_55_64.RData")
