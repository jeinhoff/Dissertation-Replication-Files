...
data <- data %>%
  select(COUNTRY, YEAR, SEX, AGE, AGE_GRP, HHNUM, ILOSTAT, EMPSTAT, MAINSTAT, CONTRHRS, HWUSUAL, HATLEV1D, ISCO88_1D, ISCO88_1DPR, NACE1_1D, NACE1_1DPR, COEFFY, COEFFHH, COEFFMOD, EXISTPR, YEARPR, SEEKREAS, WANTREAS, LEAVREAS) %>%
  filter(AGE_GRP %in% c("Y65-69", "Y70-74")) %>%
  mutate(across(everything(), as.character))

colnames(data) <- c("country", "year", "sex", "age", "age_group", "hh_id", "status_ilo", "status_emp", "status_self", "working_hours_contract", "working_hours_usual", "education", "occupation_current", "occupation_last", "industry_current", "industry_last", "weight", "weight_hh", "weight_mod", "last_employment", "last_employment_year", "seek_reas", "want_reas", "leave_reas")
...
data <- data %>%
  mutate(
    weight = ifelse(is.na(weight), weight_mod, weight),
    weight = ifelse(is.na(weight), weight_hh, weight))
...
data <- data %>%
  select(COUNTRY, YEAR, SEX, AGE, AGE_GRP, HHNUM, ILOSTAT, EMPSTAT, MAINSTAT, CONTRHRS, HWUSUAL, HATLEV1D, ISCO08_1D, ISCO08_1DPR, NACE2_1D, NACE2_1DPR, COEFFY, COEFFHH, COEFFMOD, EXISTPR, YEARPR, SEEKREAS, WANTREAS, LEAVREAS) %>%
  filter(AGE_GRP %in% c("Y65-69", "Y70-74")) %>%
  mutate(across(everything(), as.character))
...
data <- data %>% 
  mutate(country_group = case_when(
    country %in% c("FI", "NO", "SE", "DK") ~ "(1) Northern Europe",
    country %in% c("BG", "HU", "LT", "LV", "PL", "RO", "SI", "CZ", "HR", "SK", "EE") ~"(4) Central and Eastern Europe",
...
data <- data %>% 
  mutate(cohort_two = case_when(
    cohort %in% c(1909:1910) ~ "'09/'10",
    cohort %in% c(1911:1912) ~ "'11/'12",
    cohort %in% c(1913:1914) ~ "'13/'14",
    cohort %in% c(1915:1916) ~ "'15/'16",
    cohort %in% c(1917:1918) ~ "'17/'18",
    cohort %in% c(1919:1920) ~ "'19/'20",
...
data <- data %>%
  mutate(status_self = case_when(
    ...
    status_self %in% c(5, 6, 7, 8) ~ "Other",
    T ~ "missing"))
...
data <- data %>%
  mutate(seek_reas = case_when(
    ...
    seek_reas %in% c(2, 4, 5, 6, 8) ~ "Other",
    T ~ "missing"))
...
data$working_hours_usual <- ifelse(is.na(data$working_hours_usual), data$working_hours_contract, data$working_hours_usual)

```

```{r}
data <- data %>% select(country, country_code, country_group, gender, year, cohort, cohort_two, age, status, working_hours_usual, working_hours_contract, education, weight)

data <- data %>% arrange(country, gender, cohort, age)

save.image("outputs/data_raw_65_74.RData")
